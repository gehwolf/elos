...
static safuResultE_t _backendStart(elosStorageBackend_t *backend) {
    ...
    if (backend == NULL || backend->backendData == NULL) {
        ...
        result = elosInMemoryBackendStart(backend->backendData);
...
static safuResultE_t _backendPersist(elosStorageBackend_t *backend, const elosEvent_t *event) {
    ...
    if ((backend == NULL) || (event == NULL)) {
        ...
        result = elosInMemoryBackendPersist(backend->backendData, event);
        result = SAFU_RESULT_OK;
    }
...
static safuResultE_t _backendShutdown(elosStorageBackend_t *backend) {
    ...
    if (backend == NULL) {
        ...
        result = elosInMemoryBackendStop(backend->backendData);
        if (result != SAFU_RESULT_OK) {
            safuLogErr("Failed to stop dlt backend");
        }

        result = elosInMemoryBackendDeleteMembers(backend->backendData);
        if (result != SAFU_RESULT_OK) {
            safuLogErr("elosInMemoryBackendDeleteMembers failed");
        }
    }
...
static safuResultE_t _pluginLoad(elosPlugin_t *plugin) {
    ...
    if (plugin == NULL) {
        ...
        if (newBackend == NULL) {
            ...
            elosInMemoryBackendParam_t param = {0};
            int32_t size = 0;
            samconfConfigStatusE_t retVal = samconfConfigGetInt32(plugin->config, "/Config/Size", &size);
            ...
            result = elosInMemoryBackendNew((elosInMemoryBackend_t **)&newBackend->backendData, &param);
            if (result != SAFU_RESULT_OK) {
...
static safuResultE_t _pluginUnload(elosPlugin_t *plugin) {
    ...
    if (plugin == NULL) {
        ...
        elosInMemoryBackendDelete(backend->backendData);
        free(backend);
        result = SAFU_RESULT_OK;
    }
